...
public class CompleteRegistrationActivity extends AbstractCompleteActivity {
    ...
    private void attemptCompleteRegistration() {
        ...
        token = getToken();
        UserConfirmation userConfirmation = validateInput(username, password, password, token);

        if (userConfirmation != null) {
            showProgress(true);
            mnuboApi.getClientOperations().confirmUserCreation(username, userConfirmation, new MnuboApi.CompletionCallBack<Boolean>() {
                @Override
    ...
    private UserConfirmation validateInput(String username, String password, String confirmPassword, String token) {
        ...
        return new UserConfirmation(password, token);
    }
}
